...
List _myDatas = [
  {"name": 'Alarm_1', "status": true, "state": false, "system": "111s1", "registerDate" : 'aa1', "terminationDate":"1" , "timeStamp": '2020-02-09 23:44:12', 'description': '点1位置围界400米报警发生'},
  {"name": 'Alarm_2', "status": false, "state": false, "system": "111s1", "registerDate" : 'aa1', "terminationDate":"1",  "timeStamp": '2020-02-09 23:44:12', 'description': '点1位置围界400米报警发生'},
  {"name": 'Alarm_3', "status": true, "state": false, "system": "111s1", "registerDate" : 'aa1', "terminationDate":"1",  "timeStamp": '2020-02-09 23:44:12', 'description': '点1位置围界400米报警发生' },
];
...
class _MyHomePageState extends State<DataTablePage> {
  ...
  Widget _getTitleItemWidget_(bool label, double width) {
    ...
    return  Row(
      children: <Widget>[
        Container(
          child: Checkbox(
            value: _myDatas[0]["state"],
            activeColor: Colors.blue, //选中时的颜色
            onChanged:(value){
              print('---top-----checkbox------$value');
              setState(() {
                  _myDatas[0]["state"]=value;
              });
            } ,
          ),
          width: width,
  ...
  Widget _generateFirstColumnRow(BuildContext context, int index) {
    return  Row(
      ...
      children: <Widget>[
        Container(
          child:  Checkbox(
            value: _myDatas[index]["state"],
            activeColor: Colors.blue, //选中时的颜色
            onChanged:(value){
              print('--------checkbox------$value');
              setState(() {
                  _myDatas[index]["state"]=value;
              });
            },
          ),
          width: 40,
        ...
        Container(
          child: FlatButton(
            child: Text(_myDatas[index]["name"]),
            padding: EdgeInsets.fromLTRB(15, 0, 0, 0),
            onPressed: () {
              print('-name--${_myDatas[index]["name"]}');
  ...
  Widget _generateRightHandSideColumnRow(BuildContext context, int index) {
    print('---datas----${_myDatas[index]}');
    var data = _myDatas[index];
    return GestureDetector(
      child:     Row(
        children: <Widget>[
           _rowItem(data["system"], data),
           _rowItem(data["status"] ? 'Occured' : 'Accepted', data),
           _rowItem(data["registerDate"], data),
           _rowItem(data["terminationDate"], data),
          // _rowItem(data["system"], data, ()=> {print('system')}),
          // _rowItem(data["status"] ? 'Occured' : 'Accepted', data, ()=> {print('status')}),
          // _rowItem(data["registerDate"], data, ()=> {print('registerDate')}),
          // _rowItem(data["terminationDate"], data, ()=> {print('terminationDate')}),
        ],
      ),
      onTap: () {
        Navigator.of(context).pushNamed("alarmdetail_page", arguments: data);
  ...
  Widget _rowItem (String label, data) {
    return Container(
      child: Text(label),
      width: 100,
      height: 52,
  ...
  Widget _rowItembk (String label, data, callback) {
    return Container(
      child: FlatButton(
        child: Text(label),
        padding: EdgeInsets.fromLTRB(5, 0, 0, 0),
